...
function setup() {
  ...
  backgrounD = createSprite(displayWidth/2, displayHeight/2+300, displayWidth, displayHeight)
  ...
  ground = createSprite(800, 1350, displayWidth, 1)
...
function draw() {
  ...
  if (gamestate === "PLAY") {
    ...
    //image(background1, 0, displayHeight, displayWidth)
 
   camera.y = knight.y
   camera.x = knight.x

   //backgrounD.x = camera.y
   ...
   if(time > 30 && score > 20){
    ...
    else if(knight.isTouching(monster2Group)){
      lives = lives-1
...
function monsters() {
  if (frameCount % 20 === 0) {
    monster = createSprite(1600, Math.round(random(camera.y, camera.y)), 10, 10);
    monster.velocityX = -7
    monster.scale = 5
...
function monsters2() {
  if (frameCount % 55 === 0) {
    monster2 = createSprite(1600, 730, 10, 10);
...
function monsters3() {
  if (frameCount % 70 === 0) {
    monster3 = createSprite(1600, Math.round(random(540, 740)), 10, 10);
